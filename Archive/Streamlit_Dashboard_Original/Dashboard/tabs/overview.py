...
def calculate_all_risk_scores(predictions_hash: str, server_preds: Dict) -> Dict[str, float]:
    """
    Extract pre-calculated risk scores from daemon (PHASE 3 OPTIMIZATION).

    ARCHITECTURAL IMPROVEMENT:
    - Daemon calculates risk scores ONCE for all servers (proper separation of concerns)
    ...
    - Result: 270x faster + proper architecture
    ...
    for server_name, server_pred in server_preds.items():
        # EXTRACT pre-calculated risk_score from daemon (Phase 3)
...
def render(predictions: Optional[Dict], daemon_url: str = DAEMON_URL):
    ...
    PHASE 4 OPTIMIZATION: Fragment-based rendering - only reruns when needed, not on every interaction.

    Args:
        predictions: Current predictions from daemon
        daemon_url: URL of the inference daemon
    """
    # PHASE 4: Chart container reuse (50% faster chart updates)
    ...
    if predictions:
        ...
        if server_preds:
            ...
            with col1:
                ...
                st.session_state.overview_charts['risk_bar'].plotly_chart(
            ...
            with col2:
                ...
                st.session_state.overview_charts['risk_pie'].plotly_chart(
                    fig, use_container_width=True, config={'displayModeBar': False}
                )
